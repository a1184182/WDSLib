...
/*bool test;
  for(map<int,set<set<int> > >::reverse_iterator rit=Loop.rbegin();rit!=Loop.rend();++rit ){
    for(set<set<int> >::iterator sit=(rit->second).begin();sit!=(rit->second).end();++sit){
      map<int,set<set<int> > >::reverse_iterator rit2=rit;   
      rit2++;   
      test=false;
      set<int> input=*sit;
      set<set<int> >::iterator mem=sit;  
      while(rit2!=Loop.rend()){ 
          for(set<set<int> >::iterator sit2=(rit2->second).begin();sit2!=(rit2->second).end()&&!test;++sit2){
            //counter1++;
            set<int> target=*sit2;
            test=loop_diff(input,target,Loop2);
            if(test){
              //  NNL++;
               (Loop[input.size()]).erase(mem);
                input=target;
                mem=sit2;
            }
          }
...
for(map<int,set<set<int> > >::iterator it=Loop.begin();it!=Loop.end();++it ){
  if(!it->second.empty()){
    for(set<set<int> >::iterator sit=(it->second).begin();sit!=(it->second).end();++sit){
        Loop2->push_back(*sit);
    }
  } 
}
