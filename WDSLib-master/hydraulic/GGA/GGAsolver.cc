...
GGAsolver::GGAsolver(Net* p_net,Parameter *p_parameter,Flag* p_flag)
{
	...
	Gfinal=new vector<double> ();
	Ffinal=new vector<double> ();
    net=p_net;
...
//-------------------------------------------------------------------------
	...
	double *RHS=(double *) calloc(nj,sizeof(double));
	vector<map<int,double > >p(nj);
	vector<int> v;
	vector<double> value;

	vector<double> bb(nj);
	Matrix* LHS=new Matrix(nj,nj);
  	for(int i=0;i<np;i++){
  		int pSN=(*SN)[i];
  		int pEN=(*EN)[i];
  		if(pSN<nj&&pEN<nj){  
   	...
   	for(int i=0;i<nj;i++)	LHS->triple(i, i, diag[i]);
   	
   	for(unsigned int i=0;i<p.size();i++)
   		for (std::map<int,double>::iterator it=p[i].begin(); it!=p[i].end(); ++it)
   	  		LHS->triple(i,it->first,it->second);
	LHS->CCS();
	for(int i=0;i<nj;i++)	RHS[i]=((-modifier3*(*dm)[i]+bb[i])/modifier1);

	vector<double>* result=LHS->linsolve(pamd,RHS);
	delete LHS;
	free(RHS);
	return result;
}
...
void GGAsolver::amd(Result* result){
	...
	for(unsigned int i=0;i<np;i++){
		qsol->push_back(0.0);
		Gfinal->push_back(0.0);
		Ffinal->push_back(0.0);
...
//-------------------------------------------------------------------------
	...
	{  
		...
		if(flag->getFlagvalue("ConvergenceFlag")){
			...
			Eres=NORM(EresI,3);
			Cres=NORM(CresI,3);
			result->setIterResidual(Cres,Eres);
			delete EresI;
			delete CresI;
		}
	...
	for(unsigned i=0;i<pRow->size();i++){ 
		(*Gfinal)[(*pRow)[i]]=(*G)[i]*Gscale;
		(*Ffinal)[(*pRow)[i]]=(*F)[i]*Gscale;
		(*qsol)[(*pRow)[i]]=(*q)[i]*qScale;
	}
